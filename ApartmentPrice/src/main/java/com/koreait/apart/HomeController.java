...
public class HomeController {
	...
	public String result(SearchPARAM param,  Model model) {
		...
		UriComponents builder = UriComponentsBuilder.fromHttpUrl(url).queryParam("LAWD_CD", lawd_cd).queryParam("DEAL_YMD", deal_ym)
				.queryParam("numOfRows", "100").queryParam("serviceKey", decodeServiceKey).build(false);
		...
		ResponseEntity<String> respEntity = restTemplate.exchange(builder.toString(), HttpMethod.GET, entity,
				String.class);
		...
		if(list.size() > 0) {
			for(ItemDTO item : list) {
				...
				item.setDeal_amount(s.trim().replace(",", ""));
